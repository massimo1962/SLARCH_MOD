...
extern double host_latency (SLMSrecord * msr);
...
static char *qcfile       = 0;	 /* quality check file */
...
//  char message [30] ;
  ...
  do
    {
	 // sl_log (2,0,"** total seqnum: %i  \n",seqnum);
	  if (slpack){
	  seqnum_pre = sl_sequence (slpack); // prendo il precedente

	  /* Copy fixed header */
	    memcpy (&fsdh, &slpack->msrecord, sizeof(struct sl_fsdh_s));
	  }
	  sl_result = sl_collect (slconn, &slpack);
	  ptype  = sl_packettype (slpack);
      seqnum = sl_sequence (slpack);

      // ----> chk seqnum se maggiore di 1 erore -> scrivi gap
//       snprintf (message, 30,
//  		    "%04d,%02d,%02d,%02d,%.5s,%.3s,%.2s,%.2s",
...
//  		    fsdh.location);
     ...
     // if (seqnum-seqnum_pre > 1) {sl_log (2,0,"erore-buco seq_pre:%i seqnum: %i info:%s  \n",seqnum_pre,seqnum, message);}
...
packet_handler (char *msrecord, int packet_type, int seqnum, int packet_size)
{
  ...
  /* Discards bogus packets */
  if (sl_msr_parse (slconn->log, msrecord, &msr, 1, 0)) {
  ...
  if ( dsarchive && archflag ) {
    ...
    long usec = msr->fsdh.start_time.fract * 100;
...
//	  sl_log_rl (log, 1, 0, "Cannot apply microsecond offset\n");
    ...
    if(latency > 0.0){
       snprintf (message,100,
  		    "%04d.%03d.%02d:%02d:%02d.%06d [LAT] %.5s_%.3s_%.2s_%.2s_ %f",
  		    msr->fsdh.start_time.year,
  		    msr->fsdh.start_time.day,               
  		    msr->fsdh.start_time.hour,
  		    msr->fsdh.start_time.min,
  		    msr->fsdh.start_time.sec,
                    usec,
  		    msr->fsdh.station,
  		    msr->fsdh.channel,
  		    msr->fsdh.network,
  		    msr->fsdh.location, latency);
        sl_log (0,0,"%s \n", message );
        sl_qcsave (message);
    }
...
parameter_proc (int argcount, char **argvec)
{
  ...
  int qcflag        = 0;  /*quality check flag*/ 
  ...
  for (optind = 1; optind < argcount; optind++)
      ...
      else if (strncmp (argvec[optind], "-v", 2) == 0)
	{
      ...
      else if (strncmp (argvec[optind], "-Q", 3) == 0)
        {
	  qcflag = 1;
	  qcfile = getoptval(argcount, argvec, optind++);
        }   
...
addarchive ( const char *path, const char *layout )
{
  return 0;
  /*
   modified to do not write archive
   */
/*
...
usage (int level)
{
  ...
  fprintf (stderr,
	   ...
	   " -Q qcfile       Write quality check data\n"          
	   "\n"
