...
ds_streamproc (DataStream *datastream, SLMSrecord *msr, long suffix)
{
  ...
  double dsamprate;
  char channel [30] ;
  char time_channel [100];
  struct tm *time_struct;
  double lastsample_real;
  double intpart;
  ...
  if ( foundgroup != NULL )
      ...
      if ( writeloops >= 10 )
	{
      ...
      foundgroup->modtime = time (NULL);
       ...
       char buf[80];
       lastsample_real=modf(foundgroup->lastsample,&intpart);
       
       if (lastsample_real>0.0) {
                 
       }
      //TODO aggiungere condizione ed opzione corrispondente
      startepoch = sl_msr_depochstime(msr);
      sl_msr_dsamprate(msr, &dsamprate);
      time_t rawtime;
      char content[200]="";
      if((startepoch - foundgroup->lastsample) > 1.1*(1.0/dsamprate) && (foundgroup->lastsample > 0.000) ) {
          //snprintf(channel,30, "%.5s_%.3s_%.2s_%.2s_", msr->fsdh.station, msr->fsdh.channel, msr->fsdh.network, msr->fsdh.location );
          //snprintf (content,200, "%s  startepoch %f - lastsample %f = %f -- samplerate= %f period= %f\n ", channel, startepoch, foundgroup->lastsample, startepoch - foundgroup->lastsample, dsamprate , 1.0/dsamprate);
          //snprintf (content,200, "%f %s [GAP] %f\n ", foundgroup->lastsample, channel,  startepoch - foundgroup->lastsample);
        //prendere parte intera di lastsample
        char str_lastsample_real [10] ;
        snprintf(str_lastsample_real, 10, "%f", lastsample_real) ;
        if (str_lastsample_real[0] == '0') 
            memmove(str_lastsample_real, str_lastsample_real+1, strlen(str_lastsample_real));
        rawtime=intpart;  
        time_struct = gmtime((const time_t *) &rawtime);

        strftime (buf, sizeof(buf), "%Y.%j.%H:%M:%S", time_struct );
        snprintf (time_channel,100,
                    "%s%s [GAP] %.5s_%.3s_%.2s_%.2s_", 
                    buf,
                    str_lastsample_real,                               
                    msr->fsdh.station,
                    msr->fsdh.channel,
                    msr->fsdh.network,
                    msr->fsdh.location );      
          snprintf (content,200, "%s %f\n ", time_channel,  startepoch - foundgroup->lastsample);          
          sl_qcsave(content);
          //sl_log (0, 0, "[QC] Gap found! startepoch %f - lastsample %f = %f -- samplerate= %f period= %f\n ", channel, startepoch,foundgroup->lastsample,startepoch - foundgroup->lastsample, dsamprate ,1.0/dsamprate);
      }
//      else
//      {
//          sl_log (0, 0, "Continuous data found! startepoch %f - lastsample %f \n",startepoch,foundgroup->lastsample);
//      }
      //sl_log (2, 0, "ds_streamproc: Here i am\n");
